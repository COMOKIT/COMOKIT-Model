...
species Authority {
	...
	action update_monitor(AbstractActivity act, bool allowed) {
		if(act_monitor != nil){
			ask act_monitor { 
	...
	bool allows (AbstractIndividual i, AbstractActivity activity) { 
		float start <- BENCHMARK ? machine_time : 0.0;
	...
	int limitGroupActivity (AbstractIndividual i, AbstractActivity activity) { 
	...
	AbstractPolicy with_age_group_at_home_policy(AbstractPolicy p, list<pair<int,int>> age_group_at_home){
		map<AbstractIndividual,bool> age_group_allowance;
		ask all_individuals { 
	...
	AbstractPolicy create_school_work_allowance_policy (bool school, bool work) {
		create ActivitiesListingPolicy returns: result {
			allowed_activities[act_studying] <- school;
			allowed_activities[act_working] <- work;
		}
