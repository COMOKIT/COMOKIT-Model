...
import "../Models/Synthetic Population.gaml"
...
global {
	...
	reflex nb_contact_per_day when:_contact_tracking {
		_cday <- _cday + mean(all_individuals collect nb_contacts(Individual(each)));
	...
	float nb_contacts(Individual indiv) {
		...
		ask indiv {
			if is_at_home { 
				contacts <- contacts + list<Individual>(relatives) count each.is_at_home; 
...
experiment batch_Contact_Tracking parent:"Abstract Experiment" type:batch 
	...
	init {
		dataset_path <- world.build_dataset_path();
