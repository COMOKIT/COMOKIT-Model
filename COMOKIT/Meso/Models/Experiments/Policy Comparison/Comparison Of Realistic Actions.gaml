...
import "../../Entities/HospitalisationPolicy.gaml"
...
global {
			...
			default {
				ask Authority {
					if with_hospital_policy { 
						policy <- world.create_hospitalisation_policy(true, true, number_of_test_in_hospital);
					} 
					else {
	...
	action build_french_style_action {
		ask Authority { 
			...
			if with_hospital_policy { 
				policy <- combination([d, l, r, world.create_hospitalisation_policy(true, true, number_of_test_in_hospital)]);
			} 
			else { 
	...
	action build_south_corean_style_action {
		ask Authority { 
			...
			if with_hospital_policy { 
				policy <- combination([d, l, world.create_hospitalisation_policy(true, true, number_of_test_in_hospital)]);
			} 
			else { 
	...
	action build_britain_style_plan {
		ask Authority {
			...
			AbstractPolicy r <- create_positive_at_home_policy();
			if with_hospital_policy {
				policy <- combination([l, r, world.create_hospitalisation_policy(true, true, number_of_test_in_hospital)]);
			} else {
